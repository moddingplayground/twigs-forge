...
public class TwigsBiomeModifier implements BiomeModifier {
    ...
    public void modify(Holder<Biome> biome, Phase phase, ModifiableBiomeInfo.BiomeInfo.Builder builder) {
        ...
        if (phase == Phase.ADD) {
            if (biome.is(TwigsBiomeTags.SPAWNS_RHYOLITE)) {
                biomeGenerationSettingsBuilder.addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, TwigsPlacedFeatures.ORE_RHYOLITE_LOWER);
            }
            if (biome.is(TwigsBiomeTags.SPAWNS_BLOODSTONE)) {
                biomeGenerationSettingsBuilder.addFeature(GenerationStep.Decoration.TOP_LAYER_MODIFICATION, TwigsPlacedFeatures.ORE_BLOODSTONE_NETHER);
            }
            if (biome.is(TwigsBiomeTags.SPAWNS_SCHIST)) {
                biomeGenerationSettingsBuilder.addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, TwigsPlacedFeatures.ORE_SCHIST_UPPER);
                biomeGenerationSettingsBuilder.addFeature(GenerationStep.Decoration.UNDERGROUND_DECORATION, TwigsPlacedFeatures.ORE_SCHIST_LOWER);
            }
